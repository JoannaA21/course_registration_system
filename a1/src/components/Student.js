...
export const StudentInformation = ({ handleChange, handleSubmit, newquestion, questions, handleResponse, handleSubmitRes }) => {
    ...
    const handleExchange = async (course) => {
        const myexchangeCourse = { id: token.id, courseid: course.id };
        console.log(myexchangeCourse);

        //make put request to api/exchangecourse
        fetch('http://localhost:3001/exchangecourse', {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify(exchangeCourse),
        });
    ...
    const handleDrop = async (courseid) => {
        fetch('http://localhost:3001/coursedrop/${courseid}',{
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
